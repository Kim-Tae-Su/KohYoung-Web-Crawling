...
### 프로젝트 개요
본 프로젝트는 CRM(Customer Relationship Management) 시스템 내  
장비 및 고객 이력 데이터를 자동으로 수집하여  
딥러닝 학습 데이터로 활용하기 위한 데이터 수집 자동화 프로젝트이다.

CRM 시스템은 웹 UI로만 제공되며,  
...
### 시스템 아키텍처
본 시스템은 Selenium 기반 웹 UI 자동화를 통해 CRM 목록 페이지에서  
장비별 상세 페이지 링크를 수집하고,  
수집된 링크를 기반으로 상세 페이지에 접근하여  
웹 스크래핑을 통해 파일 메타데이터를 확인한 뒤  
...
CRM 시스템은 스크롤 이벤트에 따라 데이터가 동적으로 로드되며,  
개별 데이터 링크 접근 후 뒤로 이동할 경우  
페이지 상태가 초기화되는 구조였다.

이로 인해 데이터 누락 및 반복 스크롤로 인한  
성능 저하 문제가 발생하였다.
...
파일 크기 확인 없이 일괄 다운로드 시  
디스크 용량 및 네트워크 리소스 낭비 가능성이 존재하였다.
...
## 4. Web Crawling Automation Flow

CRM 웹 UI의 실제 사용자 동작 흐름을 분석하여  
다음과 같은 전체 크롤링 자동화 플로우를 설계·구현하였다.

1. 로그인 페이지 접근 및 인증 처리
2. CRM 메인 페이지 진입 후 대상 메뉴 이동
3. 장비 / 고객 이력 목록 페이지 접근
4. 목록 페이지 DOM 구조 분석을 통한 대상 데이터 Element 탐색
5. 무한 스크롤(Infinite Scroll) 방식 페이지 대응을 위한 자동 스크롤 처리
6. 스크롤 완료 시점 판단 및 전체 데이터 로딩 여부 검증
7. 로딩 완료 후 목록 내 모든 상세 페이지 링크 수집
8. 수집된 상세 페이지 링크를 CSV 파일로 저장
9. CSV 파일 기반으로 상세 페이지 링크 순차 재접근
10. 개별 상세 페이지 진입
11. 파일 메타데이터(파일명, 크기 등) 사전 조회
12. 다운로드 대상 여부 판단 (파일 크기 임계값 기준)
13. 조건을 만족하는 파일만 선택적으로 다운로드
14. 다운로드 성공 / 실패 및 예외 상황 로깅
15. 전체 수집 완료 후 결과 정리 및 종료
...
- 10MB ~ 10GB 규모의 장비 이력 데이터 약 1만 건 안정적 수집
...
본 프로젝트는 단순한 웹 크롤링 구현을 넘어  
API나 DB 접근이 불가능한 제한된 환경에서도  
...
데이터 확보 시간을 단축하고,  
이후 딥러닝 모델 개발에 집중할 수 있는 환경을 구축하였다.
